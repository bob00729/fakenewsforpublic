...
```{r Yearly Trends in GSS 1972-2016 data, echo=FALSE, message=FALSE, warning=FALSE}
# Import data
...
plot_ly(mideast_ly, x = ~MYEAR, y = ~Press, type = "scatter", mode = "lines+markers", 
...
In 2008, "markets" and other business topics dominate the written word landscape. 2008 was the year of the mortgage market collapse in the United States, so it is not surprising to find "market" the keyword of that year. For 2012 and 2016, the picture isn't as clear, although articles in 2012 seem to be focused on industrial issues, and those in 2016 seem to be concentrated around social and election issues.
...
```{r GSS, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
Mean confidence levels for variables in question are listed in popups on every layer of the map, along with mean age of respondents and percentage of foreign-born respondents for each region on the map. As can be seen, the mean age of respondents is a bit high in GSS for confidence in the press variable, so we turn to National Politics Study 2008 next, which has a more reasonable distribution of age and can be mapped by individual states.
...
```{r GSS Spatial, echo=FALSE, message=FALSE, warning=FALSE}
...
In order to dig in deeper into spatial differences among states, we used the NPS 2008 survey to create the map below. It shows the differences in Trust in the Media levels among individual states in 2008 (states with no data appear grey). An overlay highlights states that tend to trust government and overlap quite well with the states that tend to also have high levels of trust in the media, with the exceptions of West Virginia and Massachussetts. Popups show average trust in the media and in the government by state.

```{r NPS Spatial, echo=FALSE, message=FALSE, warning=FALSE}
# Import data
...
```{r WVS Spatial, results = 'hide', echo=FALSE, message=FALSE, warning=FALSE}
...
We intended to compare the individual confidence level in the press in various countries by
conducting analysis in static, geographic, and textual dimensions. Also, we were planning to
investigate possible explanatory variables that may have correlations with confidence level in the
press and visualize the findings.
...
There are more explanations of each graph in our final markdown.
